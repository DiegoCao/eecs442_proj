...
def train():
    ...
    for epoch in range(num_of_epochs):
        ...
        for i, data in enumerate(dataset.cielab_loader):
            ...
            fake_lab_images = torch.cat((l_images, fake_images), dim=1 )
            ...
            D_opt.zero_grad()
            d_loss = 0
            logits = discriminator(torch.cat([l_images, c_images], 1)).reshape(-1)
            d_real_loss = criterion(logits, ((1 - smooth) * torch.ones(batch_size)))

            logits = discriminator(torch.cat([l_images, fake_images], 1)).reshape(-1)
            d_fake_loss = criterion(logits, (torch.zeros(batch_size)))
            ...
            g_fake_loss = criterion(fake_logits, (torch.ones(batch_size)))
            g_image_distance_loss = g_lambda * reg_criterion(fake_images, c_images)
            ...
            if i % 2 == 0:
                ...
                g_running_loss = 0.0
                with torch.no_grad():
                    img_grid_real = torchvision.utils.make_grid(utils.lab2rgb(lab_images[:32]), normalize=True)
                    img_grid_fake = torchvision.utils.make_grid(utils.lab2rgb(fake_lab_images[:32]), normalize=True)
